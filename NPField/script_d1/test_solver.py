...
def resolve_paths(finetune_checkpoint: str = "") -> tuple[Path, Path, Path]:
    ...
    default_checkpoint = Path(
        os.getenv(
            "NPFIELD_CHECKPOINT",
            repo_root / "NPField" / "dataset" / "trained-models" / "NPField_D1.pth",
        )
    )
...
def test_solver(
    ...
    for growth in tf_growth_factors:
        ...
        for i in range(N + 1):
            candidate_simX[i] = acados_solver.get(i, "x")
        candidate_goal_error_m = math.hypot(
            candidate_simX[-1, 0] - x_ref_points[-1],
            candidate_simX[-1, 1] - y_ref_points[-1],
        )
    ...
    if num_map == -1:
        ...
        ax1.legend()
        ticks_x = ticker.FuncFormatter(lambda x, pos: "{0:g}".format(x / MAP_SCALE))
        ax1.xaxis.set_major_formatter(ticks_x)
        ticks_y = ticker.FuncFormatter(lambda x, pos: "{0:g}".format(x / MAP_SCALE))
        ax1.yaxis.set_major_formatter(ticks_y)

    colors_list = ["k"] * 2 + ["r"] * 2 + ["g"] * 2 + ["b"] * 2 + ["c"] * 2 + ["m"] * 2 + ["y"] * 2 + ["k"] * 2 + ["r"] * 2 + ["g"] * 2 + ["b"] * 2 + ["c"] * 2 + ["m"] * 2 + ["y"] * 2 + ["k"] * 2
    ...
    ax1.plot(
        [OBST_x[0, j] * MAP_SCALE for j in [0, 1, 2, 3, 0]],
        [OBST_y[0, j] * MAP_SCALE for j in [0, 1, 2, 3, 0]],
        color="k",
    )
    for i in range(TIME_STEPS - 1):
        if obstacle_traj is not None:
            ...
            x_obst += OBSTACLE_STEP_M * cos(theta_obst)
            y_obst += OBSTACLE_STEP_M * sin(theta_obst)
        ...
        ax1.plot(
            [OBST_x[i + 1, j] * MAP_SCALE for j in [0, 1, 2, 3, 0]],
            [OBST_y[i + 1, j] * MAP_SCALE for j in [0, 1, 2, 3, 0]],
            color=["r", "g", "b", "c", "m", "y", "k", "r", "g"][i],
        )
    path_mpc = simX
...
def gif_generate(
    ...
    def _robot_polygon(x_pos, y_pos, theta):
        s = MAP_SCALE
        x0 = x_pos + s * 0.6 * cos(theta - 0.59)
        y0 = y_pos + s * 0.6 * sin(theta - 0.59)
        x5 = x_pos + s * 0.6 * cos(theta + 0.59)
        y5 = y_pos + s * 0.6 * sin(theta + 0.59)
        x6 = x_pos - s * 0.6 * cos(theta - 0.59)
        y6 = y_pos - s * 0.6 * sin(theta - 0.59)
        x7 = x_pos - s * 0.6 * cos(theta + 0.59)
        y7 = y_pos - s * 0.6 * sin(theta + 0.59)
    ...
    OBST_y = np.zeros([4])
    if obstacle_traj is not None:
        x_obst, y_obst, theta_obst = obstacle_traj[0]
    else:
        x_obst = dyn_obst_info["initial_position"][num_map, 0]
        y_obst = dyn_obst_info["initial_position"][num_map, 1]
    ...
    for i_t in np.arange(0.0, total_time + 1e-6, 0.05):
        fig2, ax2 = plt.subplots(1, 1, figsize=(5, 5))
        frame_idx = min(int(max(i_t, 0.0) / OBSTACLE_PRED_DT), TIME_STEPS - 1)
        if nn_map_frames is not None:
            bg_map = nn_map_frames[frame_idx]
        else:
            bg_map = map_data[num_map][0]
        if potential_frames is not None:
            ax2.imshow(
                potential_frames[frame_idx].T,
                origin="lower",
            ...
            ax2.pcolor(bg_map[::-1], cmap=cmap_bw, edgecolors="w", linewidths=0.1)
        ...
        ax2.plot(
            [OBST_x[j] * MAP_SCALE for j in [0, 1, 2, 3, 0]],
            [OBST_y[j] * MAP_SCALE for j in [0, 1, 2, 3, 0]],
            color="k",
        )
        interp_x, interp_y, interp_theta = _interp_pose(path, i_t)
        poly_x, poly_y = _robot_polygon(interp_x, interp_y, interp_theta)
        ax2.plot(poly_x, poly_y, color="k")
        if k_robot < len(path) - 1 and i_t > path[k_robot, 4]:
            k_robot += 1
        if round(i_t, 1) >= t_obst and k_obst < (TIME_STEPS - 1):
            if obstacle_traj is not None:
    ...
    output_path = output_dir / f"D1_MAP_ep{num_map}_ID_{id_map}.gif"
    imageio.mimsave(str(output_path), frames, format="GIF", fps=20)


# ---------------------------------------------------------------------------
# D1-specific: map input and multi-frame embedding
# ---------------------------------------------------------------------------
...
def _draw_obstacle_on_map(base_map, x_obst, y_obst, theta_obst):
    ...
    The obstacle is drawn as occupied cells (value 100) using the same
    quadrilateral footprint used by the GIF rendering code.
...
def fill_map_inp(num_map, map_data, footprint, obstacle_traj=None):
    """Build NN input from sub-maps (future static frames).

    When obstacle_traj is provided (benchmark mode): starts from the base
    map (frame 0, no baked obstacle) and draws the obstacle at the
    benchmark-specified positions for each timestep.
    When obstacle_traj is None (dataset mode): uses the dataset's
    pre-computed sub-maps which already contain the obstacle.
    Normalization: divide by 100.0 (matching NPField_model.py / training).
    """
    map_inp = torch.zeros((TIME_STEPS, 5000))
    fp_flat = torch.tensor(footprint.flatten(), dtype=torch.float32) / 100.0

    for n in range(TIME_STEPS):
        if obstacle_traj is not None:
            frame = _draw_obstacle_on_map(
                map_data[num_map][0],
                obstacle_traj[n, 0], obstacle_traj[n, 1], obstacle_traj[n, 2],
            )
        else:
            frame = map_data[num_map][n + 1]
        sub_map = torch.tensor(frame.flatten(), dtype=torch.float32) / 100.0
        map_inp[n, :2500] = sub_map
        map_inp[n, 2500:] = fp_flat
    return map_inp


def compute_all_embeddings(model_loaded, map_inp):
    """Compute one embedding per static frame → (TIME_STEPS, 612)."""
    device = next(model_loaded.parameters()).device
    embeddings = np.zeros((TIME_STEPS, NUM_EMBEDDING_OUTPUTS))
    with torch.no_grad():
        for n in range(TIME_STEPS):
            result = model_loaded.encode_map_footprint(map_inp[n].to(device)).detach()
            embeddings[n] = result[0].cpu().numpy()
    return embeddings
...
def sample_theta_from_path(path_mpc, steps=TIME_STEPS):
    ...
    sampled_unwrapped = np.interp(
        sample_times, path_times, theta_unwrapped,
        left=theta_unwrapped[0], right=theta_unwrapped[-1],
    )
    return (sampled_unwrapped + math.pi) % (2.0 * math.pi) - math.pi


def infer_potential_grid(model_loaded, all_embeddings, heading_samples, device, chunk_size):
    """Evaluate potential over a spatial grid for each static frame."""
    ...
    with torch.no_grad():
        for pred_idx, angle in enumerate(heading_samples):
            emb = torch.tensor(
                all_embeddings[pred_idx], dtype=torch.float32, device=device,
            ).unsqueeze(0)
            emb_rep = emb.repeat(coords_t.shape[0], 1)
            theta = torch.full((coords_t.shape[0], 1), float(angle), dtype=torch.float32, device=device)
            input_batch = torch.hstack((emb_rep, coords_t, theta))
            outputs = []
            ...
            potential_frames[pred_idx] = output[:, 0].reshape(POTENTIAL_RESOLUTION, POTENTIAL_RESOLUTION)
    return potential_frames


def save_potential_gif(map_data, num_map, id_map, output_dir, potential_frames):
    frames = []
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / f"NPField_D1_potential_ep{num_map}_ID_{id_map}.gif"
...
def get_acados_solver(model_loaded, map_inp, cache_key, allow_backward=False):
    if cache_key in _SOLVER_CACHE:
        return _SOLVER_CACHE[cache_key], _EMBEDDINGS_CACHE[cache_key]
    all_emb = compute_all_embeddings(model_loaded, map_inp)
    acados_solver = create_solver.create_solver(
        model_loaded, all_emb, allow_backward=allow_backward,
    )
    _SOLVER_CACHE[cache_key] = acados_solver
    _EMBEDDINGS_CACHE[cache_key] = all_emb
    return acados_solver, all_emb
...
def run_simulation(
    ...
    map_inp = fill_map_inp(num_map, map_data, footprint, obstacle_traj=obstacle_traj)
    ...
    if obstacle_traj is not None:
        ...
        cache_key = (num_map, allow_backward)
        acados_solver, all_emb = get_acados_solver(
    ...
    if save_potential_gif_flag and save_gif:
        model_loaded.eval()
        device = next(model_loaded.parameters()).device.type
    ...
    nn_map_frames = (map_inp[:, :2500].reshape(TIME_STEPS, 50, 50) * 100).numpy()

    if save_gif:
        gif_generate(
            path_mpc, ROB_x, ROB_y, num_map, id_map, map_data, dyn_obst_info,
            output_dir, x_ref_points, y_ref_points,
            obstacle_traj=obstacle_traj,
            potential_frames=potential_frames,
            nn_map_frames=nn_map_frames,
        )
...
# ---------------------------------------------------------------------------
# Trajectory metrics (Table 1 of the paper)
# ---------------------------------------------------------------------------
        ...
        try:
            for scenario in scenarios:
                ...
                try:
                    ...
                    print(f"  FAILED: {e}")
                    metrics = {
                        "scenario_id": sid, "map_id": map_id,
                        "status": "failed", "error": str(e),
                        "time_ms": "", "path_length_m": "", "smoothness": "",
                        "aol": "", "safety_distance_m": "", "goal_error_m": "",
                        "path_efficiency": "", "mean_clearance_m": "",
                        "velocity_utilization": "", "control_energy": "",
                        "jerk": "", "spl": "",
                ...
                completed += 1
                writer.writerow([
                    metrics["scenario_id"], metrics["map_id"],
                    metrics["time_ms"], metrics["path_length_m"],
                    metrics["smoothness"], metrics["aol"],
                    metrics["safety_distance_m"], metrics["goal_error_m"],
                    metrics["path_efficiency"], metrics["mean_clearance_m"],
                    metrics["velocity_utilization"], metrics["control_energy"],
                    metrics["jerk"], metrics["spl"],
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run NPField D1 (Static-MLP) solver.")
    ...
    parser.add_argument(
        "--save-potential-gif", action="store_true",
        help="Overlay potential field on the trajectory GIF.",
    )
    parser.add_argument(
        "--potential-chunk-size", type=int, default=4096,
        help="Batch size for potential grid inference.",
    )
    parser.add_argument(
        "--test-episode", action="store_true",
        help="Run a deterministic scenario.",
    )
    parser.add_argument(
        "--finetune-checkpoint", type=str, default="",
        help="Optional path to a finetuned D1 checkpoint; overrides NPFIELD_CHECKPOINT.",
    )
    parser.add_argument(
        "--benchmark-json", type=str, default="",
...
def main():
    ...
    for i in range(args.episodes):
        if not args.test_episode:
            x_ref_points, y_ref_points, theta_0 = generate_config(
                costmap, args.map_id, args.num_orientation,
            )
            obstacle_traj = None
        run_simulation(
            num_map=args.map_id,
            ...
            potential_chunk_size=args.potential_chunk_size,
            id_map=i,
            obstacle_traj=obstacle_traj if args.test_episode else None,
            save_gif=True,
            allow_backward=args.allow_backward,
        )
