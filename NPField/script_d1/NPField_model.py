...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run NPField D1 (Static-MLP) inference and save a GIF."
    )
    parser.add_argument("episode", nargs="?", type=int, default=0, help="Episode index.")
    parser.add_argument("id_dyn", nargs="?", type=int, default=0, help="Dynamic obstacle id (unused, kept for CLI compat).")
...
def encode_map(
    ...
    """Encode a single sub-map frame into a 612-dim embedding."""
    map_inp = (
        torch.tensor(
    ...
    with torch.no_grad():
        return model.encode_map_footprint(map_inp)
...
def infer_grid(
    ...
    """Evaluate potential over a spatial grid for one embedding.

    Returns a (RESOLUTION, RESOLUTION) array.
    """
    ...
    return output[:, 0].reshape(RESOLUTION, RESOLUTION).numpy()
...
def main() -> None:
    ...
    sub_maps = data["sub_maps_all"]["submaps"]
    test_footprint = data["footprints"]["footprint_husky"]

    res_array = np.zeros((NUM_FRAMES, RESOLUTION, RESOLUTION))
    for frame_idx in range(NUM_FRAMES):
        print(f"  Encoding & inferring frame {frame_idx}/{NUM_FRAMES - 1} ...")
        test_map = sub_maps[args.episode, frame_idx]
        encoded = encode_map(model, test_map, test_footprint, args.device)
        res_array[frame_idx] = infer_grid(
            model, encoded, angle_rad, args.device, args.chunk_size
        )

    frames = []
    for tme in range(NUM_FRAMES):
        fig, (ax1, ax2) = plt.subplots(
            nrows=1, ncols=2, figsize=(15, 5), gridspec_kw={"wspace": 0.3, "hspace": 0.1}
        )
        ax1.imshow(sub_maps[args.episode, tme])
    ...
    gif_name = f"NPField_D1_ep{args.episode}_angle_{angle_tag}.gif"
