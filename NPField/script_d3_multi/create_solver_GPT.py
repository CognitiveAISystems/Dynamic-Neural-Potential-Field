...
def create_solver(model_loaded, embedding_values, allow_backward=False,
    ...
    *solver_overrides* is an optional dict that can contain keys like
    ``W_OBST``, ``LM_DAMPING``, ``NLP_MAX_ITER`` to override mpc_params
    defaults — useful for multi-obstacle scenarios that need higher cost
    weight or looser damping.
    """
    ov = solver_overrides or {}
    ...
    w_obst_val = float(ov.get("W_OBST", W_OBST))
    W_obst = np.array([w_obst_val])
    ...
    ocp.solver_options.levenberg_marquardt = float(ov.get("LM_DAMPING", LM_DAMPING))
    ocp.solver_options.nlp_solver_max_iter = int(ov.get("NLP_MAX_ITER", NLP_MAX_ITER))
    ocp.solver_options.qp_solver_iter_max = int(ov.get("QP_MAX_ITER", QP_MAX_ITER))
