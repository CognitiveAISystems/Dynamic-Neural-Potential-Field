...
def robot_model(model_loaded, embedding_values):
    ...
    class _EmbeddingWrappedModel(torch.nn.Module):
        """Wraps GPT with pre-computed obstacle embeddings.

        For N obstacles the model is evaluated N times (once per embedding)
        and the resulting potentials are summed — additive superposition.
        An explicit sequential loop is used instead of batched expand/reshape
        so that L4CasADi's autograd tracing stays robust.
        """
        ...
        def forward(self, xytheta):
            ...
            num_obst = emb.shape[0]

            emb_0 = emb[0:1].expand(xytheta.shape[0], -1)
            out_sum = self.base_model(torch.cat([emb_0, xytheta], dim=1))

            for i in range(1, num_obst):
                emb_i = emb[i:i+1].expand(xytheta.shape[0], -1)
                out_sum = out_sum + self.base_model(
                    torch.cat([emb_i, xytheta], dim=1)
                )
            return out_sum
    ...
    cost_obst = potential_l4c_at_embeding[num_prediction_steps_obst - 1]
    ...
    for j in range(num_prediction_steps_obst):
        ...
        cost_obst = if_else(cond_3, potential_l4c_at_embeding[j], cost_obst)
